...
Module ResourceWriter
    ...
    Public Function WriteResource(ByVal filename As String, ByVal bytes As Byte(), ByVal N As Integer) As Boolean

        Try
            Dim handle As IntPtr = BeginUpdateResource(filename, False)
            Dim file1 As Byte() = bytes
            Dim fileptr As IntPtr = ToPtr(file1)
            Dim res As Boolean = UpdateResource(handle, "MultiBind", N, 0, fileptr, Convert.ToUInt32(file1.Length))
